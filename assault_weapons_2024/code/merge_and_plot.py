...
if BODY == "house":
    X_AXIS_NAME = "Percent of US Congressional Districts (%)"
else:
    X_AXIS_NAME = "Percent of US Senate Districts (%)"
...
def make_plots(dfs):
    def hover_info(row, name, data_name, mean_col):
        result = (
            f"Data: {data_name}<br>"
        ...
        if name != "Republican and Conservative Voters Only":
            result += f"Lean: {row['partisan_lean (%)']:.1f}%<br>"
        result += f"Gun ownership: {row['gun_owner_fraction_right_only']:.1f}%<br>"
        return result

    def label_info(row, name, data_name, mean_col):
        result = (
            f"{data_name} ({row['district']})<br>" f"Support: {row[mean_col]:.0f}%; "
        )
        if name != "Republican and Conservative Voters Only":
            result += f"Lean: {row['partisan_lean (%)']:.0f}%; "
        result += f"Gun ownership: {row['gun_owner_fraction_right_only']:.0f}%"
        return result
    ...
    for name, df in dfs.items():
        ...
        for mean_col, stderr_col in zip(
            ...
            df["hover_info"] = df.apply(
                hover_info, args=(name, data_name, mean_col), axis=1
            )
            scatter_dict = {
                "x": df["index"],
                "mode": "markers",
        ...
        if not MERGE_PLOTS:
            ...
            for annotation in ANNOTATION_LIST:
                ...
                row = df.iloc[index]
                ic(row)
                text = label_info(row, name, data_name, data_name)
